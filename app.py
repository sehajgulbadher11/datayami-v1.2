    ...
    try:
        ...
        with col2:
            ...
            if st.button("Top 5 Rows"):
                st.session_state.messages.append({"role": "user", "content": "Show me the top 5 rows"})
            st.markdown('</div>', unsafe_allow_html=True)
            
        with col3:
            st.markdown('<div class="insight-button">', unsafe_allow_html=True)
            if st.button("Column Insights"):
                st.session_state.messages.append({"role": "user", "content": "What are the data types and missing values of the columns?"})
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        selected_columns = st.multiselect(
            "Select columns to analyze (leave empty to use all)", 
        ...
        enable_chart = st.toggle("Enable Chart Generation")
        if enable_chart:
            ...
            with chart_col3:
                y_col = st.selectbox("Y-Axis", df_filtered.columns.tolist())
                
            if st.button("Plot Chart"):
                import plotly.express as px
                from visualization import _apply_dark_theme
                try:
                    if chart_type == "Bar":
                        fig = px.bar(df_filtered, x=x_col, y=y_col, color_discrete_sequence=['#c026d3'])
                    elif chart_type == "Line":
                        fig = px.line(df_filtered, x=x_col, y=y_col, color_discrete_sequence=['#3b82f6'])
                    elif chart_type == "Pie":
                        fig = px.pie(df_filtered, names=x_col, values=y_col, color_discrete_sequence=px.colors.sequential.Purp)
                    else:
                        fig = px.scatter(df_filtered, x=x_col, y=y_col, color_discrete_sequence=['#ec4899'])
        ...
        if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
            ...
            with st.chat_message("assistant"):
                with st.spinner("Analyzing data..."):
                    result = query_data(df_filtered, user_query, st.session_state.messages)
                    
                if result.get("error"):
                    st.error(result["error"])
                elif result.get("type") == "text":
                    st.write(result["content"])
                    st.download_button("Download Latest Result", data=result["content"], file_name="result.txt", mime="text/plain", key=f"dl_txt_{len(st.session_state.messages)}")
